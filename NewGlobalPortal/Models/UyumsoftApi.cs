...
namespace NewGlobalPortal.Models
{
    public class UyumsoftApi
    {
        ...
        //{"value": {"coCode": "string","entityId": 0,}}

        public string CariGrupKodunuDonder(string CariId)
        {
            try
            {
                Yetkiler yetki = new Yetkiler();
                string kod = "-1";
                string metodUrl = PostUrl + "/UyumApi/v1/FIN/GetCoEntity";
                Uri uri = new Uri(metodUrl);
                var client = new RestClient(uri);
                var request = new RestRequest();
                request.Method = RestSharp.Method.Post;
                ...
                request.AddJsonBody("{\"value\": {\"coCode\": \"" + yetki.parametre.FirmaKodu + "\",\"entityId\": " + CariId + "}}");
                var jsonResponse = client.Execute(request);
                if (jsonResponse.IsSuccessful)
                {
                    var result = JsonConvert.DeserializeObject<UyumCariDetaylari>(jsonResponse.Content);
                    if (result.statusCode == 200)
                    {
                        foreach (var item in result.result.cO_ENTITY_M)
                        {
                            if (item.entityPriceGrpSCode!=null && item.entityPriceGrpSCode!="")
                            {
                                kod = item.entityPriceGrpSCode;
                            }
                        }
                    }
                }
        ...
        public string Cariler()
        {
            ...
            var aaaa=JsonConvert.SerializeObject(listMaster);
        ...
        public string Siparisler()
        {
            ...
            do
            {
                ...
                request.AddHeader("Authorization", tokeType + " " + token);
                request.AddHeader("UyumSecretKey", security);
                request.AddJsonBody(" { value: { id:0, entityId: " + yetki.kullanici.YetkiliOlduguCariIdleri + "},pageIndex: " + i + ",pageSize: 50}");
                var jsonResponse = client.Execute(request);
        ...
        public string Urunler()
        {
            ...
            if (tum.TumUrunlerStr=="" || tum.TumUrunlerStr==null)
            {
                Yetkiler yetki = new Yetkiler();
                ...
                do
                {
                    ...
                    request.AddHeader("Authorization", tokeType + " " + token);
                    request.AddHeader("UyumSecretKey", security);
                    request.AddJsonBody(" { value: { id:0, branchCode: '" + yetki.parametre.IsYeriKodu + "',DontWants: 'ItemImage' },pageIndex: " + i + ",pageSize: 150}");
                    // request.AddJsonBody(" { value: { id:0, branchCode: '"+ yetki.parametre.IsYeriKodu + "' },pageIndex: " + i + ",pageSize: 150}");
                    var jsonResponse = client.Execute(request);
                    ...
                    if (jsonResponse.IsSuccessful)
                    {
                        ...
                        foreach (var item in result.result)
                        {
                            ...
                            foreach (var item2 in item.itemPriceList.Where(x => x.priceListCode.Contains("#") && x.entityPriceGrpCode.Equals(yetki.kullanici.CariGrupKodu)))
                            {
        ...
        public string FiltreliUrunGetir(string itemCode)
        {
            ...
            do
            {
                ...
                request.AddHeader("Authorization", tokeType + " " + token);
                request.AddHeader("UyumSecretKey", security);
                request.AddJsonBody(" { value: { itemCode:'" + itemCode + "', branchCode: '" + yetki.parametre.IsYeriKodu + "'},pageIndex: " + i + ",pageSize: 50}");
                var jsonResponse = client.Execute(request);
                ...
                if (jsonResponse.IsSuccessful)
                {
                    ...
                    foreach (var item in result.result)
                    {
                        ...
                        foreach (var item2 in item.itemPriceList.Where(x=> x.entityPriceGrpCode.Equals(yetki.kullanici.CariGrupKodu)))
                        {
                            if (DateTime.Now < Convert.ToDateTime(item2.endDate))
                            {
                                item2.sonFiyat = (float)Math.Round(item2.unitPriceTra * ((100 - item2.disc1Rate) / 100), 2);
        ...
        public string SiparisOlustur(string baslikLogicalref, string odemeTipi, string ilgiliKisi, string tel, string detayliAdres, string city, string towN, string siparisAciklamasi, string RefNo)
        {
            ...
            try
            {
                ...
                siparis.entityId = Convert.ToInt32(yetki.kullanici.YetkiliOlduguCariIdleri);
                siparis.entityCode = FiltreliCariGetir(yetki.kullanici.YetkiliOlduguCariIdleri).result.entitY_M[0].entityCode;
                siparis.isApproveByMaster = false;
                siparis.orderStatus = "Açık";
                siparis.sourceApp = yetki.parametre.SatisSiparisKaynagi;
                siparis.docTraCode = yetki.parametre.HareketKodu;
                siparis.gnlNote1 = "Siparişi Oluşturan= " + yetki.kullanici.KullaniciAdi + " (" + yetki.kullanici.AdiSoyadi + ")";
                ...
                siparis.branchCode = yetki.parametre.IsYeriKodu;
                siparis.curCode = "TRY";
                siparis.curTra = 1;
                siparis.coCode = yetki.parametre.FirmaKodu;
                siparis.docDate = DateTime.Now;
                siparis.docNo = yetki.parametre.SatisSiparisBelgeNoBaslangici + "-" + (Convert.ToDouble(1 + "" + yetki.parametre.SatisSiparisBaslangicNo.Substring(0, yetki.parametre.SatisSiparisBaslangicNo.Length - 1)) + Convert.ToDouble(baslikLogicalref));
                ...
                foreach (var item in sepetUrunleri)
                {
                    ...
                    {
                        sourceApp = yetki.parametre.SatisSiparisKaynagi,
                        ...
                        whouseCode = yetki.parametre.DepoKodu,
                        curCode = "TRY",
                        curRateTra = 1,
                ...
                if (jsonResponse.IsSuccessful)
                {
                    ...
                    mail.EksizMailGonderme("satis@newglobalinsaat.com,info@newglobalinsaat.com,yonetim@newglobalinsaat.com", yetki.kullanici.MailAdresi, "Yeni Sipariş", siparis.docNo + " Numaralı Siparişiniz Tarafımıza Ulaşmıştır.Siparişlerim Ekranında Görebilirsiniz.");
                    result.IsSuccessful = true;
        ...
        public string TeklifOlustur(string baslikLogicalref, string odemeTipi, string ilgiliKisi, string tel, string detayliAdres, string city, string towN, string siparisAciklamasi)
        {
            ...
            try
            {
                ...
                var cariBilgileri = FiltreliCariGetir(yetki.kullanici.YetkiliOlduguCariIdleri);
                siparis.entityId = Convert.ToInt32(yetki.kullanici.YetkiliOlduguCariIdleri);
                siparis.entityCode = cariBilgileri.result.entitY_M[0].entityCode;
                siparis.isApproveByMaster = false;
                siparis.orderStatus = "Açık";
                siparis.sourceApp = yetki.parametre.TeklifKaynagi;
                siparis.docTraCode = yetki.parametre.TeklifHareketKod;
                siparis.gnlNote1 = "Teklifi Oluşturan= " + yetki.kullanici.KullaniciAdi + " (" + yetki.kullanici.AdiSoyadi + ")";
                //siparis.gnlNote2 = "Virman Fişi DocNo= " + RefNo;
                siparis.shippingAddress1 = ilgiliKisi;
                siparis.shippingAddress2 = tel;
                siparis.shippingAddress3 = detayliAdres;
                ...
                siparis.docNo = yetki.parametre.TeklifBelgeNoBaslangici + "-" + (Convert.ToDouble(1 + "" + yetki.parametre.TeklifBaslangicNo.Substring(0, yetki.parametre.TeklifBaslangicNo.Length - 1)) + Convert.ToDouble(baslikLogicalref));
                if (sepetUrunleri.Count() > 0)
                {
                ...
                foreach (var item in sepetUrunleri)
                {
                    siparis.details[sayac] = new TeklifDetail
                    {
                        ...
                        sourceApp = yetki.parametre.TeklifKaynagi,
                        itemId = Convert.ToInt32(item.UrunId),
                        lineType = "S",
                        qty = Convert.ToInt32(item.Miktar),
                        unitPrice = Convert.ToDouble(item.IndirimsizFiyat),
                ...
                if (jsonResponse.IsSuccessful)
                {
                    ...
                    MailIslemleri mail = new MailIslemleri();
                    mail.EksizMailGonderme("satis@newglobalinsaat.com,info@newglobalinsaat.com,yonetim@newglobalinsaat.com", yetki.kullanici.MailAdresi, "Yeni Onay Bekleyen Sipariş", siparis.docNo + " Numaralı Siparişiniz Tarafımıza Ulaşmıştır.Onaylandıktan Sonra Siparişlerim Ekranında Görebilirsiniz.");
                    result.IsSuccessful = true;
                    result.Content = jsonResponse.Content;
                }
                else
        ...
        public RestResponse VirmanOlustur(double Tutar, string RefNo)
        {
            ...
            virmanBilgileri.coCode = yetki.parametre.FirmaKodu;
            virmanBilgileri.branchCode = yetki.parametre.IsYeriKodu;
            ...
            virmanBilgileri.details[0] = new UyumVirmanDetail
            {
                ...
                finDCardId = Convert.ToInt32(yetki.kullanici.YetkiliOlduguCariIdleri),
                amtTra = Tutar,
                amt = Tutar,
                curCode = "TRY",
                curRateTra = 1,
                ...
                if (jsonResponse.IsSuccessful)
                {
                    ...
                    if (result.result.iteM_M_M != null)
                        foreach (var item in result.result.iteM_M_M)
                        {
                            ...
                            if (item.entityId.ToString() == yetki.kullanici.YetkiliOlduguCariIdleri)
                            {
                    ...
                    if (result.result.iteM_D != null)
                        foreach (var item in result.result.iteM_D)
                        {
                            ...
                            if (item.entityId == yetki.kullanici.YetkiliOlduguCariIdleri)
                            {
                                listItemD.Add(item);
                            }
